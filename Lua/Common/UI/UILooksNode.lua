...
function UILooksNode:Reset(  )
	...
	if self.lastData.showType == UILooksNode.ShowType.Role then
		if not IsNull(self.bodyTrans) then
			...
			self.cacheRole[self.curShowCareer..","..self.curShowBody] = self.bodyTrans
		end
...
function UILooksNode:LoadRole( )
	if (self.curShowCareer and self.curShowCareer==self.data.career) and (self.curShowBody and self.curShowBody == self.data.body) then
		return
	end
	-- if not IsNull(self.bodyTrans) then
	-- 	self.bodyTrans.gameObject:SetActive(false)
	...
	local cacheTrans = self.cacheRole[self.data.career..","..self.data.body]
    ...
    local on_load_body_succeed = function ( bodyObj )
    	...
    	self.bodyTrans.localScale = Vector3(1.2,1.2,1.2)
