...
function BagView:OnLoad(  )
	...
	-- 增加红点回调
	RedpointTree.SetCallBack(RedpointTree.NodeNames.Bag_stone,'Bag_stone',function ()
        self:UpdateRedpoint(RedpointTree.GetRedpointCnt(RedpointTree.NodeNames.Bag_stone))
    end)
	self:UpdateRedpoint(RedpointTree.GetRedpointCnt(RedpointTree.NodeNames.Bag_stone))
end
...
function BagView:AddEvents(  )
	local on_click = function ( click_obj )
		...
		elseif self.swallow_obj == click_obj then
			Message:Show("功能未开启")
		elseif self.stone_obj == click_obj then
			RedpointTree.ChangeRedpointCnt(RedpointTree.NodeNames.Bag_stone,-1)
		end
	end
	UI.BindClickEvent(self.swallow_obj, on_click)
	UI.BindClickEvent(self.sort_obj, on_click)
	UI.BindClickEvent(self.stone_obj, on_click)
...
function BagView:UpdateRedpoint(redCnt)
	print("11111111111111222222222222222222")
	local redpoint = self.stone_obj.transform:Find('redpoint')
    if not redpoint then
        return
    end
	print("3333333333333334444444444444")
    if redCnt > 0 then
		print("46546545465465465")
