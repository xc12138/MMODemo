...
function LoginCreateRoleView:UpdateRoleMesh( sex )
	local show_data = {
		...
		body = sex == 1 and 3 or 3,
		-- hair = 1,
		career = sex == 1 and 1 or 1,
		canRotate = true,
	}
	self.roleUILooksNode = self.roleUILooksNode or UILooksNode.New(self.select_role_con)
	self.roleUILooksNode:SetData(show_data)
	self.select_body = show_data.body or self.select_body
end

function LoginCreateRoleView:OnClickRandomName(  )
	local nick_name = nil
	math.randomseed(os.clock() * 10000)
	self.surname = self.surname or {"月见","星野","绯夜","白川","九条","雾岛","青空","八重","神无","赤瞳","黑羽","苍叶","时雨","七海","龙崎"}
	self.forename_man = self.forename_man or {"苍介","龙牙","瞬夜","光一","圭介","煌斗","光司","彻也","隼人","鬼刃","幻月","朔夜","岚牙","烬灭","虚空","夜枭"}
	self.forename_woman = self.forename_woman or {"菜菜","成美","赤穗","初音","雏子","千夏","琉璃","诗织","羽咲","樱雪","心菜","星罗","蝶祈","幽奈","绯绪"}
	-- self.symbol = self.symbol or {"·","&","丶","^"}
...
function LoginCreateRoleView:AddEvents(  )
	local on_click = function ( click_btn )
        ...
        elseif click_btn == self.create_role_obj then
	        ...
	        NetDispatcher:SendMessage("account_create_role", {name=self.role_name_input.text, career=self.select_sex, body=self.select_body}, on_ack)
		end
	end
