...
function MainUIMenuView:OnLoad(  )
	...
	self:AddEvents()
    -- 增加红点
    RedpointTree.SetCallBack(RedpointTree.NodeNames.Bag,'Bag',function ()
        self:UpdateRedpoint(RedpointTree.GetRedpointCnt(RedpointTree.NodeNames.Bag))
    end)
...
function MainUIMenuView:UpdateRedpoint(redCnt)
    local redpoint = self.bag_obj.transform:Find('redpoint')
