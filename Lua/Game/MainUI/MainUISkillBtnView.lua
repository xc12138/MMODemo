...
function MainUISkillBtnView:AddEvents(  )
	local on_click = function ( click_btn )
		...
		elseif click_btn == self.attack_obj then
			...
			local currentTime = math.floor(CS.UnityEngine.Time.time * 1000)
			if currentTime - self.lastClickTime < self.cdTime then
				return
			end
		...
		if skillID and skillID ~= 0 then
			...
			if not item then
				...
				item.SetCDEndTime = function(item, cdEndTime)
					...
					item.cdRingTime = math.floor((cdEndTime-curTime)/1000)
					item.countdown:CountdownByEndTime(cdEndTime, function(leftTime)
						local curTime = Time:GetServerTime()
						local nowTime = cdEndTime - curTime
						if nowTime > 0 then
							item.left_time_txt.text = math.floor(nowTime/1000)
							local percent = math.floor(nowTime/1000)/item.cdRingTime
