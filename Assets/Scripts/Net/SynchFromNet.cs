    ...
    public void OnAckSceneObjInfoChange(SprotoTypeBase result)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            ...
            for (int info_index = 0; info_index < info_len; info_index++)
            {
                ...
                // Debug.Log("cur_change_info.key : "+cur_change_info.key.ToString() +" uid"+uid.ToString()+" value:"+cur_change_info.value.ToString());
                // Debug.Log("ContainsKey:"+changeFuncDic.ContainsKey((SceneInfoKey)cur_change_info.key).ToString());
                // Debug.Log("scene_obj:"+(scene_obj!=Entity.Null).ToString());
